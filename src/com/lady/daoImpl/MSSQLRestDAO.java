...
public class MSSQLRestDAO implements RestDAO{
	...
	public int insertRest(Rest rest) {
		try {
			...
			if(rest.getChangedBy() != 0) {
				PreparedStatement stmt = con.prepareStatement("insert into [dbo].[REST] (Rest_Date, Rest_Type, Rest_Description, Emp_ID, Is_Changed, Changed_By) values (?, ?, ?, ?, ?, ?)");
				...
				PreparedStatement stmt = con.prepareStatement("insert into [dbo].[REST] (Rest_Date, Rest_Type, Rest_Description, Emp_ID, Is_Changed) values (?, ?, ?, ?, ?)");
				stmt.setString(1, rest.getRestDate());
				stmt.setString(2, rest.getRestType());
				stmt.setString(3, rest.getRestDescription());
				stmt.setInt(4, rest.getEmployeeID());
			...
			PreparedStatement stmt2 = con.prepareStatement("select Rest_ID from [dbo].[REST] order by Rest_ID desc");
	...
	public void deleteRest(int restId) {
		try {
			PreparedStatement stmt = con.prepareStatement("delete from [dbo].[REST] where Rest_ID=?");
	...
	public void updateRest(int restId, Rest rest) {
		try {
			PreparedStatement stmt = con.prepareStatement("update [dbo].[REST] set Rest_Date=? and Rest_Type=? and Rest_Description=? and Is_Changed=? and Changed_By=? WHERE Rest_ID=?");
			stmt.setString(1, rest.getRestDate());
			stmt.setString(2, rest.getRestType());
			stmt.setString(3, rest.getRestDescription());
...
//	}
	...
	public List<Rest> findRestFromEmployeeId(int year, int month, int employeeId) {
		...
		try {
			...
			for(int i=space; i<space+totalDate; i++) {
				...
				stmt = con.prepareStatement("select * from [dbo].[rest] where Rest_Date=? and Emp_ID=?");
	...
	public int findRestId(int employeeId, String date) {
		try {
			...
			PreparedStatement stmt = con.prepareStatement("select Rest_ID from [dbo].[REST] where Emp_ID=? and Rest_Date=?");
			stmt.setInt(1, employeeId);
	...
	public void changeIsChanged(int restId, int whoChange) {
		try {
			PreparedStatement stmt = con.prepareStatement("update [dbo].[REST] set Is_Changed=? and Changed_By=? WHERE Rest_ID=?");
			stmt.setInt(1, 1);
